...
3. 注意事项和常见问题解决
   ...
   3. 只有当“窗口聚焦”的时才会获取文本进行朗读，以及翻页监听。
   4. 翻页自动播放的逻辑是，当窗口聚焦的时候获取窗口标题，对比上一次和本次标题，若不一样则进行播放。
   5. 播放时会在悬浮窗显示当前播放的文本长度和前十个字符。
   6. 如果你发现翻页自动播放失效，请点击主窗口并保持预览模式 (ESC) 这样尝试聚焦。然后检查 3.2、3.3、3.4 三个条件是否满足，若检测到不同的窗口标题将会执行 3.5 条内容后成功播放内容。
   ...
   7. 如果发现悬浮窗的预览的文本发生改变或出现，但没有播放，可以使用 A 按钮重试，重试不行可以检查控制台是否打印 Timout 超时字样，如果打印则是网络环境的问题，可以更换模型或者禁用或修改 VPN 等手段解决。
   8. 如果发现播放速度延迟大于 3 秒，一般为网络环境问题，可以根据需要取舍等待，或者请根据第 3.7 条提供方案解决。
   9. 如果程序控制台报错，且翻页自动播放失效。可以尝试右键悬浮条-菜单-重启监听解决。
